...
void exec_linear_regression_from_csv(const char *filename, double learning_rate, int max_iterations, double tolerance, const char *method, const char *regularization, double lambda)
{
    ...
    fprintf(stdout, "Dimensiones de los datos: %d filas x %d columnas\n\n" RESET_COLOR, csv_data->data->rows, 2);

    fprintf(stdout, CYAN_COLOR "Algoritmo Regresion Lineal\n");

    fprintf(stdout, GREEN_COLOR "\nRegresion lineal ajustada correctamente (learning rate=%.2f, iter max=%d, tol=%.5f)\n\n" RESET_COLOR, learning_rate, max_iterations, tolerance);

    fprintf(stdout, YELLOW_COLOR "Primeras 5 muestras:\n\n" RESET_COLOR);
    int mostrar = csv_data->data->rows < 5 ? csv_data->data->rows : 5;
    for (int i = 0; i < mostrar; i++)
        fprintf(stdout, "Muestra %d: [%g] -> %g\n", i, csv_data->data->data[i][0], csv_data->labels->data[i][0]);
    printf("\n");
    ...
    export_results_to_csv(predictions, csv_data->labels, csv_data->data, "stats/resultados_lr.csv");

    fprintf(stdout, GREEN_COLOR "\nResultados exportados a stats/resultados_lr.csv\n\n" RESET_COLOR);

    int status = system("python3 python/plot_regresion_lineal.py");
    if (status == -1)
        python_script_error(__FILE__, __LINE__, "python/plot_regresion_lineal.py");

    fprintf(stdout, GREEN_COLOR "Grafico generado exportado a plots/regresion_lineal.png\n\n" RESET_COLOR);
...
void export_results_to_csv(Matrix *predictions, Matrix *y, Matrix *X, const char *filename)
{
    ...
    if (!file)
        open_file_error(__FILE__, __LINE__, filepath);
