...
void exec_linear_regression(CSVData *csv_data, double learning_rate, int max_iterations, double tolerance)
{
    ...
    compare_batch_vs_minibatch(X_train, y_train, X_test, y_test, learning_rate, max_iterations, tolerance, LR_REGULARIZATION, LR_LAMBDA);
    free(means);
    free(stds);
    matrix_free(y_pred);
...
void linear_regression_fit_minibatch(LinearRegression *lr, Matrix *X, Matrix *y, RegressionMetrics **metrics, int batch_size, const char *regularization, double lambda)
{
    ...
    *metrics = (RegressionMetrics *)malloc(sizeof(RegressionMetrics));
    ...
    (*metrics)->mse_history = (double *)malloc(lr->max_iterations * sizeof(double));
    (*metrics)->cost_history = (double *)malloc(lr->max_iterations * sizeof(double));
    if (!(*metrics)->mse_history || !(*metrics)->cost_history) { regression_metrics_free(*metrics); *metrics = NULL; return; }
    double prev_mse = DBL_MAX;
    int iteration = 0;
    for (iteration = 0; iteration < lr->max_iterations; iteration++) {
        for (int b = 0; b < n_batches; b++) {
            ...
            for (int i = 0; i < lr->weights->rows; i++) {
                double reg_term = 0.0;
                if (strcmp(regularization, "ridge") == 0) reg_term = lambda * lr->weights->data[i][0];
                else if (strcmp(regularization, "lasso") == 0) reg_term = lambda * (lr->weights->data[i][0] > 0 ? 1 : -1);
            ...
            matrix_free(errors); matrix_free(X_transpose); matrix_free(weight_gradients); matrix_free(predictions); matrix_free(X_batch); matrix_free(y_batch);
        }
...
void compare_batch_vs_minibatch(Matrix *X_train, Matrix *y_train, Matrix *X_test, Matrix *y_test, double learning_rate, int max_iterations, double tolerance, const char *regularization, double lambda)
{
    ...
    printf(CYAN_COLOR "\n╔══════════════════════════════════════════════════════════════╗\n");
    printf("║        COMPARACION: BATCH VS MINI-BATCH GRADIENT DESCENT   ║\n");
    printf("╚══════════════════════════════════════════════════════════════╝\n" RESET_COLOR);
    printf("\n%-20s %-12s %-12s %-12s %-12s\n", "Metodo", "MSE", "MAE", "R2", "Tiempo(s)");
    printf("%-20s %-12.4f %-12.4f %-12.4f %-12.4f\n", "Batch", mse_batch, mae_batch, r2_batch, time_batch);
    printf("%-20s %-12.4f %-12.4f %-12.4f %-12.4f\n", "Mini-batch", mse_mb, mae_mb, r2_mb, time_mb);
    matrix_free(y_pred_batch); matrix_free(y_pred_mb);
    linear_regression_free(lr_batch); linear_regression_free(lr_mb);
    regression_metrics_free(metrics_batch); regression_metrics_free(metrics_minibatch);
}
...
void gradient_descent_step(LinearRegression *lr, Matrix *X, Matrix *y, Matrix *predictions)
{
    ...
    if (!errors) return;
    Matrix *X_transpose = matrix_transpose(X);
    if (!X_transpose) { matrix_free(errors); return; }
    Matrix *weight_gradients = matrix_multiply(X_transpose, errors);
    if (!weight_gradients) { matrix_free(errors); matrix_free(X_transpose); return; }
    for (int i = 0; i < lr->weights->rows; i++)
